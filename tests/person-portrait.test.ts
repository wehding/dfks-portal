...
test("vælger kun et portræt, når brugeren aktivt har valgt det", () => {
  const candidate = "https://dfi-dam.qonqord.cloud/preview/example.jpg";
  assert.equal(selectPortraitUrl(candidate, [candidate]), candidate);
  assert.equal(selectPortraitUrl(null, [candidate]), null);
  assert.equal(selectPortraitUrl(undefined, [candidate]), null);
  assert.equal(selectPortraitUrl("https://example.com/portrait.jpg", [candidate]), null);
});

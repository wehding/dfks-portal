...
export async function confirmContractEpisodes(input: {
  ...
  const { data: contract } = await db.from("contracts")
    .select("id,org_id,rights_holder_id,work_id,works(type)")
  ...
  await db.from("contracts").update({ season_number: seasonNumber, episode_numbers: input.entireSeason ? null : episodeNumbers }).eq("id", contract.id);

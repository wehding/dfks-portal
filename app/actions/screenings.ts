...
export async function fetchScreeningSourceRowsForBatch(batchKey: string) {
  ...
  const { data, error } = await db
    ...
    .select("id, title, normalized_title, channel, screening_date, broadcast_time, duration_minutes, view_count, season, episode, episode_title, production_year, category, genre, description, production_countries, directors, actors, sort_status, vaerk_type, sorted_by, sorted_at")
    .eq("org_id", orgId)

...
/* eslint-disable @typescript-eslint/no-explicit-any -- Supabase case relations are rendered at this client boundary. */
...
import { getMemberEntitlementCase, sendEntitlementCaseMessage, uploadEntitlementEvidence } from "@/app/actions/rights-entitlement-cases"
...
export default function MemberEntitlementCasePage() {
  ...
  const position = item.withheld_beneficiary_positions
  const evidence = item.rights_entitlement_evidence ?? []
  const thread = Array.isArray(item.member_message_threads) ? item.member_message_threads[0] : item.member_message_threads
  const messages = [...(thread?.member_messages ?? [])].sort((a, b) => a.created_at.localeCompare(b.created_at))
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <PortalPageHeader title={item.works?.title ?? "Rettighedssag"} subtitle={`${item.right_type.toUpperCase()} · mulig rettighedsposition`} />
      ...
      <div className="grid gap-3 rounded-lg border p-4 sm:grid-cols-3">
        ...
        <div><p className="text-xs text-muted-foreground">Mulig andel</p><p className="font-medium">{(Number(position?.withheld_amount ?? 0) / 100).toLocaleString("da-DK", { style: "currency", currency: position?.currency ?? "DKK" })}</p></div>
        <div><p className="text-xs text-muted-foreground">Kravsfrist</p><p className="font-medium">{position?.rights_work_allocations?.claim_deadline ?? "—"}</p></div>
      </div>
      {!closed && (
      ...
      <div className="space-y-3 rounded-lg border p-4"><h2 className="font-medium">Indsendte dokumenter</h2>{evidence.length ? evidence.map((entry: any) => <div key={entry.id} className="flex justify-between border-t pt-2 text-sm"><span>{entry.original_filename}</span><Badge variant="secondary">{entry.review_status}</Badge></div>) : <p className="text-sm text-muted-foreground">Ingen dokumenter indsendt endnu.</p>}</div>
      <div className="space-y-3 rounded-lg border p-4"><h2 className="flex items-center gap-2 font-medium"><MessageSquare className="h-4 w-4" />Beskeder om sagen</h2>{messages.map((entry: any) => <div key={entry.id} className={`rounded-md p-3 text-sm ${entry.author_role === "member" ? "ml-8 bg-primary/10" : "mr-8 bg-muted"}`}><p>{entry.body}</p><p className="mt-1 text-xs text-muted-foreground">{new Date(entry.created_at).toLocaleString("da-DK")}</p></div>)}<div className="flex gap-2"><Textarea value={message} onChange={event => setMessage(event.target.value)} placeholder="Skriv til administrator om denne rettighed…" /><Button variant="outline" disabled={!message.trim()} onClick={async () => { const result = await sendEntitlementCaseMessage(caseId, message); if (result.success) { setMessage(""); await load() } else toast.error(result.error ?? "Beskeden kunne ikke sendes") }}>Send</Button></div></div>
    </div>
  )
}

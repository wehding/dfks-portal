    ...
    return (
        <div className="space-y-8 max-w-2xl">
            ...
            <section id="statistik" className="scroll-mt-20 rounded-lg border">
                <div className="border-b px-5 py-4"><h2 className="font-medium">Statistikindstillinger</h2><p className="mt-1 text-sm text-muted-foreground">{statisticsOptions?.profile.isOrganisationMember ? "Som medlem er du oplyst om, at foreningen bruger overordnede kontrakt- og lønoplysninger til anonymiseret statistikarbejde." : "Det er frivilligt at bidrage. Du kan altid ændre valget igen."}</p></div>
                <div className="space-y-4 p-5">
                    {statisticsOptions?.profile.isOrganisationMember ? (
                        <div className="rounded-md border bg-muted/40 px-3 py-3 text-sm">
                            <span className="block font-medium">Du deltager som medlem</span>
                            <span className="text-muted-foreground">Dine data bruges kun i aggregerede resultater med mindst {statisticsOptions?.minimumGroupSize ?? 5} forskellige deltagere.</span>
                        </div>
                    ) : (
                        <div className="grid gap-2">
                            <label className="flex items-start gap-3 rounded-md border px-3 py-3"><input type="radio" name="profile-share-statistics" className="mt-1 h-4 w-4" checked={shareStatistics} onChange={() => setShareStatistics(true)} /><span><span className="block font-medium">Ja, jeg bidrager til fælles lønstatistik</span><span className="text-sm text-muted-foreground">Dine data bruges kun i aggregerede resultater med mindst {statisticsOptions?.minimumGroupSize ?? 5} forskellige deltagere.</span></span></label>
                            <label className="flex items-start gap-3 rounded-md border px-3 py-3"><input type="radio" name="profile-share-statistics" className="mt-1 h-4 w-4" checked={!shareStatistics} onChange={() => setShareStatistics(false)} /><span><span className="block font-medium">Nej tak</span><span className="text-sm text-muted-foreground">Brug kun mine kontrakter som dokumentation for rettigheder og udbetalinger.</span></span></label>
                        </div>
                    )}

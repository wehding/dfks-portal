...
${SOURCES_SCHEMA_PROMPT},
    ...
    "postProductionSupplement": "ugentligt tillæg for efterarbejde/postproduktion som tal i kr. hvis det er særskilt aftalt; ellers null. Beløbet må ikke indgå i salary eller personalSupplement.",
    "otherSupplements": "string or null",
    "workingWeeks": null,
    "svod": "true hvis SVOD/streaming-rettigheder er udtrykkeligt i behold. For A-lønskontrakter kan en gældende overenskomst dække forholdet. For leverandørkontrakter skal retten stå udtrykkeligt; en overenskomstreference er ikke nok.",
    "copydan": "true hvis Copydan-vederlag eller Copydan-forbehold er udtrykkeligt i behold. For leverandørkontrakter må en overenskomstreference ikke i sig selv sætte feltet til true.",
    ...
    "agreementReferenceStatus": "yes når kontrakten direkte henviser til De4-fiktion, FAF, FAF-dokumentar, DJ TV eller Metal som aftalegrundlag; no ved udtrykkelig afvisning; ellers unknown",
    "holidayPayRate": "${HOLIDAY_PAY_RATE_RULE}",
    "betaRate": "${BETA_RATE_RULE}",
    "specialNotes": "KUN vilkår der er USÆDVANLIGE og konkret afviger fra det normale FOR DEN PÅGÆLDENDE KONTRAKTTYPE. Skriv max 2-3 sætninger. En overenskomstreference i en leverandørkontrakt registreres som et individuelt kontraktvilkår, men leverandøren er ikke overenskomstdækket, og pension/rettigheder må ikke udledes automatisk. Null hvis ingen reelt usædvanlige vilkår."
  },
...
- Hvis en A-lønskontrakt henviser til "overenskomsten" uden at specificere hvilken, kræves manuel kontrol. Gæt ikke aftalen.
...
- VIGTIGT — Leverandørkontrakter er ikke dækket af overenskomster. En reference registreres kun som et individuelt kontraktvilkår. Sæt collectiveAgreement til false, og udled aldrig pension, Copydan, SVOD eller andre rettigheder alene fra referencen.
- VIGTIGT — Alt-inklusivt honorar: For leverandørkontrakter er det NORMALT og FORVENTET at honoraret er alt-inklusivt og dækker pension, feriepenge og sociale udgifter — flag IKKE dette som usædvanligt eller problematisk. Det er standardvilkåret for B2B-kontrakter. Sæt pensionPercent, holidayPayRate og betaRate til null, når de kun omtales som inkluderet uden særskilt sats. Hvis leverandørkontrakten udtrykkeligt aftaler en særskilt pensionsprocent, udtrækkes den som et individuelt kontraktvilkår, aldrig som overenskomstpension. Flag kun hvis en A-lønskontrakt angiver at honoraret er alt-inklusivt — det er usædvanligt i den kontrakttype. For leverandørkontrakter: ingen warning for alt-inklusivt honorar.
...
export async function screenPortalContract(contractText: string, availableRoles: string[] = []): Promise<PortalScreeningResult> {
    ...
    const resp = await fetch("/api/screen", {
        ...
        body: JSON.stringify({
            mode: "portal",
            contractText: safeText.slice(0, 30000),
            availableRoles,
        }),
    })
    if (!resp.ok) {
...
export async function screenContract(
    ...
    const response = await fetch("/api/screen", {
        ...
        body: JSON.stringify({
            mode: "full",
            contractText: safeText.slice(0, 40000),
        }),
    })

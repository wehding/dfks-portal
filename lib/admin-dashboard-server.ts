...
type CachedResponseTime = {
  expiresAt: number;
  data: ReturnType<typeof calculateResponseTimeStats>;
};
const responseTimesCache = new Map<string, CachedResponseTime>();

async function getCachedResponseTimes(db: ReturnType<typeof createServiceClient>, orgId: string, since: string) {
  const cached = responseTimesCache.get(orgId);
  const now = Date.now();
  ...
  const calculated = calculateResponseTimeStats(events, since);
  responseTimesCache.set(orgId, {
    expiresAt: now + RESPONSE_TIME_CACHE_TTL_MS,
    data: calculated,
  });
  return calculated;
}
...
export async function loadAdminDashboardMetrics(orgId: string, userId: string): Promise<AdminDashboardMetrics> {
  ...
  const [
    ...
    contractUnread, workUnread, screeningUnread, responseTimes,
  ] = await Promise.all([
    ...
    getCachedResponseTimes(db, orgId, since),
  ]);

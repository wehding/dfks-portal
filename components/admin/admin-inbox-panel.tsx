...
export function AdminInboxPanel() {
  ...
  const visibleRecipients = useMemo(
    () => hasRecipientQuery ? filterInboxRecipients(recipients, recipientQuery) : [],
    [hasRecipientQuery, recipients, recipientQuery],
  );
  ...
  return <div className="grid gap-6 xl:grid-cols-2">
    <Card><CardHeader><CardTitle>{t("admin.inbox.newMessage")}</CardTitle></CardHeader><CardContent className="space-y-4">
      <div className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-2"><span className="text-sm font-medium">{t("admin.inbox.recipients", { count: selectedRecipients.length })}</span><div className="flex gap-1">{hasRecipientQuery && <Button type="button" variant="ghost" size="sm" disabled={!visibleRecipients.length} onClick={() => setSelectedRecipients(current => selectVisibleRecipientIds(current, visibleRecipients.map(recipient => recipient.id)))}>{t("admin.inbox.selectVisible", { count: visibleRecipients.length })}</Button>}{selectedRecipients.length > 0 && <Button type="button" variant="ghost" size="sm" onClick={() => setSelectedRecipients([])}>{t("common.deselectAll")}</Button>}</div></div>

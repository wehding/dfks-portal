...
const COPY = {
    da: {
        ...
        noUsage: "Der er endnu ingen præcise forbrugsdata. Målingen starter efter migration og deployment.",
    ...
    en: {
        ...
        noUsage: "No exact usage data exists yet. Metering starts after migration and deployment.",
...
export function AiUsageModelsTab() {
    ...
    return <div className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-semibold">{text.title}</h2><p className="text-sm text-muted-foreground">{text.subtitle}</p></div>{data.caller.role === "superadmin" && data.organisations.length > 0 && <div className="w-64 space-y-1"><Label>{locale === "en" ? "Organisation" : "Organisation"}</Label><Select value={orgFilter} onValueChange={setOrgFilter}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{locale === "en" ? "All organisations" : "Alle organisationer"}</SelectItem>{data.organisations.map(org => <SelectItem key={org.id} value={org.id}>{org.name}</SelectItem>)}</SelectContent></Select></div>}</div>
        ...
        <Card><CardHeader><CardTitle>{text.models}</CardTitle>{!data.caller.canEdit && <CardDescription>{text.readOnly}</CardDescription>}</CardHeader>
            <CardContent className="grid gap-5 lg:grid-cols-2">{(["contract_extraction", "contract_advice"] as UseCase[]).map(useCase => {
                ...
                return <div key={useCase} className="space-y-3 rounded-lg border p-4">
                    ...
                    }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{options.map(option => <SelectItem key={option.model} value={`${option.provider}/${option.model}`}>{option.label}</SelectItem>)}</SelectContent></Select></div>
                    {draft.provider === "anthropic" && <div className="flex items-start justify-between gap-4"><div><Label htmlFor={`cache-${useCase}`}>{text.caching}</Label><p className="text-xs text-muted-foreground">{text.cachingHelp}</p></div><Switch id={`cache-${useCase}`} disabled={!data.caller.canEdit} checked={draft.promptCachingEnabled} onCheckedChange={checked => setDrafts(current => current ? { ...current, [useCase]: { ...current[useCase], promptCachingEnabled: checked } } : current)} /></div>}
                    {data.caller.canEdit && <Button onClick={() => save(useCase)} disabled={saving === useCase} className="gap-2">{saving === useCase ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{text.save}</Button>}
                </div>
            })}</CardContent>
        </Card>

        <Card><CardHeader><CardTitle>{text.comparison}</CardTitle><CardDescription>{text.batchHelp}</CardDescription></CardHeader><CardContent>
            {filteredEvents.length === 0 ? <p className="text-sm text-muted-foreground">{text.noUsage}</p> : <Table><TableHeader><TableRow><TableHead>{text.model}</TableHead><TableHead>{text.extraction}</TableHead><TableHead>{text.advice}</TableHead><TableHead>{text.standard}</TableHead><TableHead>{text.batch}</TableHead></TableRow></TableHeader><TableBody>{data.models.map(model => {

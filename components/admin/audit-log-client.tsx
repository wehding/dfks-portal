...
import { AlertTriangle, Download, Loader2, RefreshCcw, Search } from "lucide-react";
...
const SOURCE_LABELS = { portal: "Medlemsportal", admin: "Administration", api: "API", cron: "Planlagt job", import: "Import", database: "Database" } as const;
const ROLE_LABELS: Record<string, string> = { superadmin: "Superadmin", admin: "Admin", "org-admin": "Organisationsadmin", jurist: "Jurist", viewer: "Læseadgang", member: "Medlem" };
const OUTCOME_LABELS = { success: "Gennemført", denied: "Afvist", failed: "Fejlet", partial: "Delvist gennemført" } as const;
...
export function AuditLogClient({ embedded = false }: { embedded?: boolean }) {
  ...
  const resetFilters = () => setFilters({ query: "", from: "", to: "", orgId: "", actorUserId: "", role: "", action: "", entityType: "", source: "", targetMemberUuid: "", purposeCode: "", systemComponent: "", outcome: "" });
  ...
  const exportAuditLog = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`/api/admin/audit-log/export${queryString ? `?${queryString}` : ""}`, { cache: "no-store" });
      if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? t("audit.error"));
      const blobUrl = URL.createObjectURL(await response.blob());
      const link = document.createElement("a");
      link.href = blobUrl;
      link.download = `dfks-auditlog-${new Date().toISOString().slice(0, 10)}.csv`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.setTimeout(() => URL.revokeObjectURL(blobUrl), 0);
      toast.success(locale === "da" ? "Auditloggen er eksporteret." : "The audit log was exported.");
    } catch (exportError) {
      const message = exportError instanceof Error ? exportError.message : t("audit.error");
      setError(message);
      toast.error(message);
    } finally {
      setExporting(false);
  ...
  return (
    <div className={embedded ? "space-y-6" : "space-y-6 p-4 sm:p-6"}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Button variant="outline" disabled={exporting || invalidPeriod} onClick={() => void exportAuditLog()}>{exporting ? <Loader2 className="size-4 animate-spin" /> : <Download className="size-4" />}{t("audit.export")}</Button>
      </div>
      ...
      <section aria-label={t("audit.filters")} className="grid gap-2 rounded-lg border bg-card p-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <select className={selectClass} aria-label={t("audit.role")} value={filters.role} onChange={event => setFilter("role", event.target.value)}><option value="">{t("audit.allRoles")}</option>{["superadmin", "admin", "org-admin", "jurist", "viewer", "member"].map(role => <option key={role} value={role}>{ROLE_LABELS[role] ?? role}</option>)}</select>
        ...
        <select className={selectClass} aria-label={t("audit.source")} value={filters.source} onChange={event => setFilter("source", event.target.value)}><option value="">{t("audit.allSources")}</option>{AUDIT_SOURCES.map(source => <option key={source} value={source}>{locale === "da" ? SOURCE_LABELS[source] : source}</option>)}</select>
        ...
        <select className={selectClass} aria-label="Resultat" value={filters.outcome} onChange={event => setFilter("outcome", event.target.value)}><option value="">{locale === "da" ? "Alle resultater" : "All outcomes"}</option>{Object.entries(OUTCOME_LABELS).map(([value, label]) => <option key={value} value={value}>{locale === "da" ? label : value}</option>)}</select>
        ...
        <Button type="button" variant="ghost" disabled={!activeFilterCount} onClick={resetFilters}>{locale === "da" ? `Nulstil filtre${activeFilterCount ? ` (${activeFilterCount})` : ""}` : `Reset filters${activeFilterCount ? ` (${activeFilterCount})` : ""}`}</Button>
      </section>

      {invalidPeriod && <p role="alert" className="text-sm text-destructive">{locale === "da" ? "Til-dato skal være den samme som eller senere end fra-dato." : "The to-date must be the same as or later than the from-date."}</p>}
      ...
      <div className="overflow-hidden rounded-lg border bg-card" aria-busy={loading}>
        ...
        {items.map(event => (
          <button key={event.id} type="button" onClick={() => setSelected(event)} className="grid w-full gap-1 border-b px-4 py-3 text-left transition-colors last:border-0 hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring md:grid-cols-[10rem_1fr_9rem_1fr_2fr] md:items-center md:gap-3">
            ...
            <span className="flex flex-wrap gap-1"><Badge variant="outline" className="w-fit">{actionLabels[event.action] ?? event.action}</Badge>{event.outcome !== "success" && <Badge variant="destructive" className="w-fit"><AlertTriangle />{OUTCOME_LABELS[event.outcome]}</Badge>}</span>
      ...
      {!loading && error && <div className="text-center"><Button variant="outline" onClick={() => void load()}><RefreshCcw />{locale === "da" ? "Prøv igen" : "Try again"}</Button></div>}
      ...
      <Dialog open={Boolean(selected)} onOpenChange={open => !open && setSelected(null)}>
        <DialogContent className="sm:max-w-2xl">
          ...
          {selected && <div className="space-y-4 text-sm">
            <dl className="grid grid-cols-[8rem_1fr] gap-2"><dt className="text-muted-foreground">{t("audit.actor")}</dt><dd>{selected.actorDisplayName || selected.actorEmail || t("audit.system")}</dd><dt className="text-muted-foreground">{t("audit.role")}</dt><dd>{selected.actorRole ? ROLE_LABELS[selected.actorRole] ?? selected.actorRole : "—"}</dd><dt className="text-muted-foreground">{t("audit.source")}</dt><dd>{locale === "da" ? SOURCE_LABELS[selected.source] : selected.source}</dd><dt className="text-muted-foreground">Resultat</dt><dd><Badge variant={selected.outcome === "success" ? "outline" : "destructive"}>{OUTCOME_LABELS[selected.outcome]}</Badge>{selected.errorCode && <span className="ml-2 font-mono text-xs">{selected.errorCode}</span>}</dd><dt className="text-muted-foreground">Medlem</dt><dd className="break-all text-xs">{data?.members.find(member => member.id === selected.targetMemberUuid)?.name ?? selected.targetMemberUuid ?? "—"}</dd><dt className="text-muted-foreground">Formål</dt><dd>{selected.purposeCode || "—"}</dd><dt className="text-muted-foreground">Retsgrundlag</dt><dd>{selected.legalBasis || "—"}</dd><dt className="text-muted-foreground">Komponent</dt><dd>{selected.systemComponent || "—"}</dd><dt className="text-muted-foreground">Datakategorier</dt><dd>{selected.dataCategories.join(", ") || "—"}</dd><dt className="text-muted-foreground">Integritet</dt><dd><Badge variant={selected.integrityValid ? "outline" : "destructive"}>#{selected.sequenceNo} · {selected.integrityValid ? "Verificeret" : "Integritetsfejl"}</Badge></dd><dt className="text-muted-foreground">{t("audit.organisation")}</dt><dd>{selected.organisations.map(org => org.name).join(", ") || "—"}</dd><dt className="text-muted-foreground">Request-id</dt><dd className="break-all font-mono text-xs">{selected.requestId || "—"}</dd><dt className="text-muted-foreground">{t("audit.correlation")}</dt><dd className="break-all font-mono text-xs">{selected.correlationId || "—"}</dd>{data?.callerRole === "superadmin" && <><dt className="text-muted-foreground">IP-adresse</dt><dd className="font-mono text-xs">{selected.ipAddress || "—"}</dd></>}</dl>

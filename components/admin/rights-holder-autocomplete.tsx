...
export function RightsHolderAutocomplete({ options, value, onChange, placeholder = "Søg rettighedshaver…" }: {
  ...
  if (selected) return <div className="flex min-h-10 items-center justify-between gap-3 rounded-md bg-muted px-3 py-2 text-sm">
    ...
    <button type="button" className="shrink-0 text-xs text-muted-foreground hover:text-foreground" onClick={() => { setQuery(""); onChange(""); }}>Fjern</button>
  </div>;
  ...
  return <div className="relative">
    ...
    {query && <button type="button" className="absolute right-3 top-2.5 text-muted-foreground" aria-label="Ryd rettighedshaver" onMouseDown={event => event.preventDefault()} onClick={() => setQuery("")}><X className="h-4 w-4" /></button>}

...
export function AdminDashboard({ metrics, notice }: { metrics: AdminDashboardMetrics; notice?: string | null }) {
  ...
  const shortcuts = [
    ...
    { id: "work-shares", href: "/admin/vaerker?shareTasks=1", icon: Scale, label: "Afstem arbejdsandele", description: "Gennemgå medlemmers procentbud og eventuelle konflikter på værker.", tasks: metrics.tasks.workShareCases, messages: 0 },
  ...
  return <div className="max-w-5xl space-y-8">
    ...
    <section className="space-y-3" aria-labelledby="response-time-title">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        {metrics.responseTimes.unansweredCount && metrics.responseTimes.oldestUnansweredThreadId ? <Link href={`/admin?thread=${encodeURIComponent(metrics.responseTimes.oldestUnansweredThreadId)}#messages`} className="rounded-lg border border-amber-300 bg-amber-50 p-4 transition-colors hover:bg-amber-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring dark:bg-amber-950/20 dark:hover:bg-amber-950/35"><p className="text-xs text-muted-foreground">{t("admin.dashboard.unanswered")}</p><p className="mt-1 text-2xl font-semibold">{metrics.responseTimes.unansweredCount}</p><p className="mt-2 text-xs font-medium text-amber-800 dark:text-amber-300">Åbn ældste ubesvarede henvendelse →</p></Link> : <div className="rounded-lg border p-4"><p className="text-xs text-muted-foreground">{t("admin.dashboard.unanswered")}</p><p className="mt-1 text-2xl font-semibold">{metrics.responseTimes.unansweredCount}</p></div>}
      </div>
    </section>

alter table public.organisations
  ...
  add column if not exists beta_invite_text text,
  add column if not exists beta_default_duration_days integer not null default 10;

alter table public.organisations
  drop constraint if exists organisations_beta_default_duration_days_check;

alter table public.organisations
  add constraint organisations_beta_default_duration_days_check
  check (beta_default_duration_days between 1 and 365);

alter table public.org_affiliations
  add column if not exists beta_tester_since timestamptz,
  add column if not exists beta_designated_by_user_id uuid references auth.users(id) on delete set null,
  add column if not exists beta_last_invite_sent_at timestamptz,
  add column if not exists beta_last_period_start_date date,
  add column if not exists beta_last_period_end_date date;

alter table public.org_affiliations
  drop constraint if exists org_affiliations_beta_period_check;

alter table public.org_affiliations
  add constraint org_affiliations_beta_period_check check (
    beta_last_period_start_date is null
    or beta_last_period_end_date is null
    or beta_last_period_end_date > beta_last_period_start_date
  );

create index if not exists org_affiliations_beta_testers_idx
  on public.org_affiliations (org_id, beta_tester_since, rights_holder_id)
  where beta_tester_since is not null;

comment on column public.organisations.beta_default_duration_days is
  'Informativ standardperiode for betainvitationer. Udløb ændrer aldrig brugerens betatesterstatus eller adgang.';
comment on column public.org_affiliations.beta_tester_since is
  'Vedvarende organisationsafgrænset betatestermarkering. Fjernes kun ved en eksplicit administratorhandling.';
comment on column public.org_affiliations.beta_last_period_end_date is
  'Informativ dato fra seneste betainvitation; bruges ikke til automatisk udløb eller adgangskontrol.';
...
begin
  ...
  event_id := public.append_audit_event_v2(
    ...
    p_source => 'admin',
    p_metadata => jsonb_build_object('invitationType', 'beta', 'emailDelivered', p_email_delivered, 'linkType', p_link_type, 'betaTester', p_enabled),

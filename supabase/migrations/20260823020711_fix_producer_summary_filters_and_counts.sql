-- Keep producer-type and association filters independent, and pre-aggregate
-- relation counts before joining them to producers. Summary cards are based on
-- the entire filtered result rather than the current page.
...
contract_aggregates as (
  select employer_id, count(distinct id)::integer as contract_count,
    bool_or(status = 'kladde') as has_draft, max(created_at) as latest_contract
  from contract_links group by employer_id
),
work_links as (
  select w.id, w.employer_id, w.created_at
  from public.works w
  where w.org_id = target_org_id and w.employer_id is not null
  union
  select w.id, we.employer_id, w.created_at
...
work_aggregates as (
  select employer_id, count(distinct id)::integer as work_count, max(created_at) as latest_work
  from work_links group by employer_id
),
base as (
  select
    e.id, e.name, e.parent_id, parent.name as parent_name, e.dfi_company_id, e.broadcaster_id,
    case when coalesce(ca.has_draft, false) then 'attention'
         when coalesce(ca.contract_count, 0) > 0 or coalesce(wa.work_count, 0) > 0 then 'active'
         else 'inactive' end as status,
    coalesce(wa.work_count, 0) as work_count,
    coalesce(ca.contract_count, 0) as contract_count,
    greatest(wa.latest_work, ca.latest_contract, e.created_at) as latest_activity,
    case when broadcaster.id is null then null else jsonb_build_object(
      'name', broadcaster.name, 'logo_path', broadcaster.logo_path, 'content_type', broadcaster.content_type
    ) end as broadcasters,
    coalesce((
      select jsonb_agg(jsonb_build_object(
        'relation_id', ept.id, 'id', pt.id, 'code', pt.code, 'name', pt.name,
        'origin', pt.origin, 'source', ept.source, 'membership_type', ept.membership_type
      ) order by pt.name)
    ...
    coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', ept.id, 'group_code', pt.code, 'group_label', pt.name,
  ...
  left join public.broadcasters broadcaster on broadcaster.id = e.broadcaster_id
  left join contract_aggregates ca on ca.employer_id = e.id
  left join work_aggregates wa on wa.employer_id = e.id
  where e.merged_into_id is null and e.archived_at is null
    ...
    and (rights_holder_filter is null or
      exists (select 1 from contract_links cl where cl.employer_id = e.id and cl.rights_holder_id = rights_holder_filter) or
      exists (
        select 1 from work_links wl
        join public.work_assignments assignment on assignment.work_id = wl.id and assignment.org_id = target_org_id
        where wl.employer_id = e.id and assignment.rights_holder_id = rights_holder_filter
      )
    )
),
filtered as (
...
summary as (
  select
    ...
    count(*) filter (where association_memberships @> '[{"membership_type":"ordinary"}]'::jsonb)::integer as ordinary_count,
    count(*) filter (where association_memberships @> '[{"membership_type":"associate"}]'::jsonb and not association_memberships @> '[{"membership_type":"ordinary"}]'::jsonb)::integer as associate_count,
    count(*) filter (where jsonb_array_length(association_memberships) > 0 and not association_memberships @> '[{"membership_type":"ordinary"}]'::jsonb and not association_memberships @> '[{"membership_type":"associate"}]'::jsonb)::integer as unknown_count,
    coalesce(sum(work_count), 0)::integer as work_count,
    coalesce(sum(contract_count), 0)::integer as contract_count
  from filtered
)
select jsonb_build_object(
  'rows', coalesce((select jsonb_agg(to_jsonb(o)) from ordered o), '[]'::jsonb),
  'filteredCount', (select filtered_count from summary),

...
-- One deliberately narrow repair. If the latest validation no longer has the
-- inspected values, this update is a no-op rather than guessing at new data.
with latest_validation as (
  select validation.id
  from public.contract_validations validation
  where validation.contract_id = '3cee6fb3-ab6b-44ee-9690-cbe95ad621c0'::uuid
  order by validation.validated_at desc nulls last, validation.created_at desc
  limit 1
)
update public.contract_validations validation
set extracted_data = jsonb_set(
  validation.extracted_data,
  '{otherSupplements}',
  jsonb_build_array(jsonb_build_object(
    'category', 'efterarbejde',
    'amount', 2000,
    'unit', 'pr. uge',
    'note', 'Fast tillæg for efterarbejde',
    'sourceText', validation.extracted_data->>'otherSupplements'
  )),
  true
)
where validation.id = (select id from latest_validation)
  and validation.extracted_data->>'salary' = '9249'
  and validation.extracted_data->>'personalSupplement' = '2751'
  and jsonb_typeof(validation.extracted_data->'otherSupplements') = 'string'
  and lower(validation.extracted_data->>'otherSupplements') like '%efterarbejde%'
  and (
    validation.extracted_data->>'otherSupplements' like '%2000%'
    or validation.extracted_data->>'otherSupplements' like '%2.000%'
  );

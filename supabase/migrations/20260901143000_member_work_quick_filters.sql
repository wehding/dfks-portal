-- Hurtigfiltre til Mine værker. Funktionen forbliver server-only og
-- returnerer kun nøgler til den side, medlemmet allerede må se.
...
as $$
  ...
  combined as (select * from assignment_groups union all select * from scope_groups),
  grouped_values as (
    ...
    select grouped.*,
      exists (select 1 from public.contracts contract
        where contract.org_id = p_org_id and contract.rights_holder_id = p_rights_holder_id
          ...
          and (contract.work_id = any(grouped.work_ids)
            or (grouped.parent_work_id is not null and contract.work_id = grouped.parent_work_id and contract.season_number = grouped.season_number))) as has_contract,
      exists (select 1 from public.member_series_episode_scopes scope
        where scope.id = any(grouped.scope_ids) and scope.status = 'pending') as missing_episodes,
      ...
      or exists (select 1 from public.work_share_participants participant
        ...
        where participant.org_id = p_org_id and participant.rights_holder_id = p_rights_holder_id
          ...
          and (participant.work_id = any(grouped.work_ids) or participant.work_id = grouped.parent_work_id)) as unresolved_shares,
      exists (select 1 from public.work_change_requests request
        where request.org_id = p_org_id and request.requested_by_rights_holder_id = p_rights_holder_id
          and request.work_id = any(grouped.work_ids) and request.status = 'pending') as has_pending,
      exists (select 1 from public.work_change_requests request
        where request.org_id = p_org_id and request.requested_by_rights_holder_id = p_rights_holder_id
          and request.work_id = any(grouped.work_ids) and request.status = 'rejected') as has_rejected,
      exists (select 1 from public.work_change_requests request
        ...
        where request.org_id = p_org_id and request.requested_by_rights_holder_id = p_rights_holder_id
          ...
          and comment.member_read_at is null) as has_message
    from grouped
  ),
  filtered as (
    select * from enriched
    where (nullif(btrim(p_search), '') is null or title ilike '%' || btrim(p_search) || '%')
  ...
  counts as (select (select count(*) from filtered) filtered_count, (select count(*) from grouped) total_count),
  paged as (
